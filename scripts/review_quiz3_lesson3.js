...
const lesson3Path = path.join(__dirname, '..', 'data', 'lesson', 'lesson3.json');
const grammar3Path = path.join(__dirname, '..', 'data', 'grammar', 'grammar3.json');

const quiz = JSON.parse(fs.readFileSync(quizPath, 'utf8'));
const lesson3 = JSON.parse(fs.readFileSync(lesson3Path, 'utf8'));
const grammar3 = JSON.parse(fs.readFileSync(grammar3Path, 'utf8'));

const vocabByHiragana = new Map();
const vocabByKanji = new Map();

lesson3.forEach(item => {
  const vocab = {
...
function checkLessonMatch(text) {
  ...
  for (const word of words) {
    if (!findVocabInfo(word) && !['の', 'は', 'が', 'を', 'に', 'へ', 'と', 'から', 'まで', 'も', 'か', 'です', 'ですか', 'ではありません', 'じゃありません', 'ください', '～', '～かい', '～えん'].includes(word)) {
...
quiz.forEach((item) => {
  ...
  if (!questionCheck.valid) {
    ...
    detail.explanation += `Từ không thuộc bài 3 trong question: ${questionCheck.invalidWords.join(', ')}. `;
    hasError = true;
  ...
  if (item.options) {
    item.options.forEach((opt, idx) => {
      ...
      if (!optCheck.valid) {
        detail.errors.push('lesson_mismatch');
        detail.explanation += `Từ không thuộc bài 3 trong option ${idx + 1}: ${optCheck.invalidWords.join(', ')}. `;
        hasError = true;
