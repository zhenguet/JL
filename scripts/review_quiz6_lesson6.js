...
const quizPath = path.join(__dirname, '..', 'data', 'quiz', 'quiz6.json');
const lesson6Path = path.join(__dirname, '..', 'data', 'lesson', 'lesson6.json');
const grammar6Path = path.join(__dirname, '..', 'data', 'grammar', 'grammar6.json');

const quiz = JSON.parse(fs.readFileSync(quizPath, 'utf8').replace(/^\uFEFF/, ''));
const lesson6 = JSON.parse(fs.readFileSync(lesson6Path, 'utf8').replace(/^\uFEFF/, ''));
const grammar6 = JSON.parse(fs.readFileSync(grammar6Path, 'utf8').replace(/^\uFEFF/, ''));

const vocabByHiragana = new Map();
const vocabByKanji = new Map();

lesson6.forEach(item => {
  const vocab = {
...
function checkLessonMatch(text) {
  ...
  for (const word of words) {
    if (!findVocabInfo(word) && !['の', 'は', 'が', 'を', 'に', 'へ', 'と', 'から', 'まで', 'も', 'か', 'です', 'ですか', 'ではありません', 'じゃありません', 'で', 'どこ', 'いつ', 'だれ', 'なに', 'なん', 'よ', '×', 'Ｘ', 'で', 'なに', 'なん'].includes(word)) {
...
quiz.forEach((item) => {
  ...
  if (!questionCheck.valid) {
    ...
    detail.explanation += `Từ không thuộc bài 6 trong question: ${questionCheck.invalidWords.join(', ')}. `;
    hasError = true;
  ...
  if (item.options) {
    item.options.forEach((opt, idx) => {
      ...
      if (!optCheck.valid) {
        detail.errors.push('lesson_mismatch');
        detail.explanation += `Từ không thuộc bài 6 trong option ${idx + 1}: ${optCheck.invalidWords.join(', ')}. `;
        hasError = true;
