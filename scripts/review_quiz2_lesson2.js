...
const grammar2Path = path.join(__dirname, '..', 'data', 'grammar', 'grammar2.json');

const quiz = JSON.parse(fs.readFileSync(quizPath, 'utf8'));
const lesson2 = JSON.parse(fs.readFileSync(lesson2Path, 'utf8'));
const grammar2 = JSON.parse(fs.readFileSync(grammar2Path, 'utf8'));
...
grammar2.forEach(item => {
  if (item.structure) {
    grammarPatterns.add(item.structure);
  }
});
...
function checkLessonMatch(text) {
  ...
  for (const word of words) {
    if (!findVocabInfo(word) && !['の', 'は', 'が', 'を', 'に', 'へ', 'と', 'から', 'まで', 'も', 'か', 'です', 'ですか', 'ではありません', 'じゃありません'].includes(word)) {
...
quiz.forEach((item) => {
  ...
  if (!questionCheck.valid) {
    ...
    detail.explanation += `Từ không thuộc bài 2 trong question: ${questionCheck.invalidWords.join(', ')}. `;
    hasError = true;
  ...
  if (item.options) {
    item.options.forEach((opt, idx) => {
      ...
      if (!optCheck.valid) {
        detail.errors.push('lesson_mismatch');
        detail.explanation += `Từ không thuộc bài 2 trong option ${idx + 1}: ${optCheck.invalidWords.join(', ')}. `;
        hasError = true;
