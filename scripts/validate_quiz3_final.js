...
if (lessonContent.charCodeAt(0) === 0xFEFF) {
  lessonContent = lessonContent.slice(1);
}
const lessonData = JSON.parse(lessonContent);

let quizContent = fs.readFileSync(quizFile, 'utf-8');
if (quizContent.charCodeAt(0) === 0xFEFF) {
  quizContent = quizContent.slice(1);
}
...
const commonWords = new Set(['です', 'ですか', 'は', 'が', 'を', 'に', 'で', 'から', 'の', 'も', 'と', 'や', 'か', 'ね', 'よ', 'さん', 'ちゃん', 'くん', 'じん', 'さい', 'なん', 'だれ', 'どなた', 'おいくつ', 'なんさい', 'どちら', 'どこ', 'いつ', 'どう', 'どんな', 'これ', 'それ', 'あれ', 'この', 'その', 'あの', 'どの', 'どれ', 'はい', 'いいえ', 'わたし', 'あなた', 'あのひと', 'あのかた', 'みなさん', 'はじめまして', 'どうぞよろしく', 'おなまえは', 'しつれいですが', 'こちらは', 'からきました', 'じゃ', 'でも', 'そして', 'しかし', 'でも', 'から', 'まで', 'へ', 'と', 'どうぞ', 'ください', 'すみません', 'ありがとう', 'どういたしまして', 'お願いします', '見せてください']);
  ...
  quizData.forEach((item, index) => {
    ...
    if (item.question && item.options && item.correctAnswer >= 0 && item.correctAnswer < item.options.length) {
      ...
      item.options.forEach(opt => {
        extractJapaneseWords(String(opt)).forEach(w => optionWords.add(w));
      });
      ...
      const correctValue = String(item.options[item.correctAnswer]);
...
fs.writeFileSync(path.join(__dirname, '..', 'quiz3_validation_result.json'), output, 'utf-8');
console.log(output);
